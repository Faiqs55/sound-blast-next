...
const BigCard = ({song}) => {
  return (
    <Link href={`songs/play/${song.slug}`} className='group w-full md:w-[45%] lg:w-[30%] rounded-lg overflow-hidden relative cursor-pointer'>
        <img className='w-full' src={song.img} alt="" />
        <div className='absolute w-full h-full top-0 left-0 bg-[#22222264] z-10 p-5'>
          <div className='flex items-center gap-2'>
            <div className='relative min-w-[60px] md:min-w-[80px] max-w-[60px] md:max-w-[80px] rounded-md overflow-hidden'>
            <img className='w-full' src={artistData[0].img} alt="" />
            <h3 className='absolute left-2 bottom-2 z-10 text-[8px] font-semibold md:font-normal md:text-xs'>{artistData[0].name}</h3>
            <div className='bg-[#22222265] absolute top-0 left-0 w-full h-full'></div>
            </div>
            <div>
                <h3 className='font-bold md:text-xl'>{song.title}</h3>
            </div>
          </div>
